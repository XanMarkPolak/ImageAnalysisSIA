...
def test_segment_image_set_obj_by_nir_valid_input():
    ...
    result1, result2 = segment_image_set_obj_by_nir(image_vis, image_nir, True, 1.0, 2.0, 4.0)
...
def test_segment_image_set_obj_by_nir_invalid_input():
    result1, result2 = segment_image_set_obj_by_nir(None, None, True, 1.0, 2.0, 4.0)
...
def test_segment_image_set_obj_by_nir_remove_small_dark_objects():
    ...
    # This object is 1 pixel smaller than MIN_OBJECT_WIDTH, so should be removed.
    image_nir[45:50, 45:45+SysConfigSIA.MIN_OBJECT_WIDTH-1] = 0
    result1, result2 = segment_image_set_obj_by_nir(image_vis, image_nir, True, 1.0, 2.0, 4.0)

    assert isinstance(result1, np.ndarray)
    assert np.sum(result1) == 0  # Check if the object was removed
    assert isinstance(result2, np.ndarray)
    assert np.sum(result2) == 0  # No light object


# Test if the function correctly leaves the same small dark object as was used in the test
# test_segment_image_set_obj_by_nir_remove_small_dark_objects() because of a scale factor of 0.5.
def test_segment_image_set_obj_by_nir_leave_small_dark_objects_downscaled():
    image_vis = np.ones((100, 100, 3), dtype=np.uint8)
    image_nir = np.ones((100, 100), dtype=np.uint8) * 255
    # Add a small object in the center
    # This object is 1 pixel smaller than MIN_OBJECT_WIDTH, so should be removed.
    image_nir[45:50, 45:45+SysConfigSIA.MIN_OBJECT_WIDTH-1] = 0
    result1, result2 = segment_image_set_obj_by_nir(image_vis, image_nir, True, 0.5, 2.0, 4.0)

    assert isinstance(result1, np.ndarray)
    assert np.sum(result1) == 255*(50-45)*(SysConfigSIA.MIN_OBJECT_WIDTH-1)  # Check if the object was removed
    assert isinstance(result2, np.ndarray)
    assert np.sum(result2) == 0  # No light object
...
def test_segment_image_set_by_vis_img_valid_input():
    image_vis = np.ones((100, 100, 3), dtype=np.uint8)  # Example RGB image
    image_nir = np.ones((100, 100), dtype=np.uint8) * 255  # Example NIR image
    result1, result2 = segment_image_set_by_vis_img(image_vis, image_nir, True, 2.0, 4.0)
    assert isinstance(result1, np.ndarray)
    assert result1.shape == image_nir.shape
    assert isinstance(result2, np.ndarray)
    assert result2.shape == image_nir.shape
...
def test_segment_image_set_by_vis_img_invalid_input():
    result1, result2 = segment_image_set_by_vis_img(None, None, True, 2.0, 4.0)
    assert result1 is None
    assert result2 is None
